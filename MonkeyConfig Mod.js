...
function MonkeyConfig(data) {
  ...
  function render() {
    ...
    data.buttons.forEach(btn => {
      html += '<td>';
      if (btn === 'save') html += `<button type="button" id="__MonkeyConfig_button_save"><img src="data:image/png;base64,${MonkeyConfig.res.icons.save}" alt="Save" onerror="this.style.display=\'none\'"/> Save</button>`;
      else if (btn === 'reset') html += `<button type="button" id="__MonkeyConfig_button_reset"><img src="data:image/png;base64,${MonkeyConfig.res.icons.reset}" alt="Reset" onerror="this.style.display=\'none\'"/> Reset</button>`;
      else if (btn === 'close') html += `<button type="button" id="__MonkeyConfig_button_close"><img src="data:image/png;base64,${MonkeyConfig.res.icons.close}" alt="Close" onerror="this.style.display=\'none\'"/> Close</button>`;
      else if (btn === 'reload') html += `<button type="button" id="__MonkeyConfig_button_reload"><img src="data:image/png;base64,${MonkeyConfig.res.icons.reload}" alt="Reload" onerror="this.style.display=\'none\'"/> Reload</button>`;
      else if (btn === 'homepage') html += `<button type="button" id="__MonkeyConfig_button_homepage"><img src="data:image/png;base64,${MonkeyConfig.res.icons.home}" alt="Homepage" onerror="this.style.display=\'none\'"/> Homepage</button>`;
      html += '</td>';});
  ...
  function update() {
    ...
    if (!root) return;
    for (let key in params) {
      const elem = root.querySelector(`[name="${key}"]`),
        param = params[key];
      if (!elem) continue;
  ...
  function open() {
    ...
    if (!container || shadowRoot.querySelector('.__MonkeyConfig_overlay').offsetHeight === 0) {
      ...
      log(`Iframe actual applied dimensions - Width: ${iframeAppliedWidth}px, Height: ${iframeAppliedHeight}px`);} else {openDone(shadowRoot);}}
  function close() {
